...
from Entities.game_components import Chip, Dice, IGameComponent
...
class MonopolyBoard(BoardGame):
    def __init__(self, game_context: "GameContext | None" = None) -> None:
        if game_context:
            ...
            self._players_list: list["MonopolyPlayer"] = game_context.players  # type: ignore
    ...
    def add_game_item(self, new_component: IGameComponent) -> None:
        ...
        if not self._validate_items(new_component):
            raise ValueError(
                "Invalid game component for Monopoly, must be a Dice or Chip"
            )
