...
class MafiaLogic(GameLogic):
    ...
    def can_start_game(self) -> bool:
        ...
        for player in self.__PLAYER_LIST:
            if (
                player.card.item_name != self.__CIVILIAN
                and player.card.item_name != self.__MAFIA
            ):
                self._send_to_ui(
    ...
    def give_roles(self) -> None:
        ...
        roles = [self.__CIVILIAN] * (PLAYER_AMOUNT - MAFIA_AMOUNT) + [
            self.__MAFIA
        ] * MAFIA_AMOUNT
    ...
    def is_game_over(self) -> bool:
        ...
        civilian_count = sum(
            1
            for player in self.__PLAYER_LIST
            if player.card.item_name == self.__CIVILIAN
        )
            ...
            for target_player in self.__PLAYER_LIST:
                if target_player not in self._board.mafias:
                    ...
                    if want_kill.lower() in ("y", "yes"):
                        naming = f"{target_player.name}_{target_player.surname}"
                        self._board.night_votes[naming] = (
                            self._board.night_votes.get(naming, 0) + 1
                        )
                        break
