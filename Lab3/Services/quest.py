...
from Lab3.Services.certificate import CertificateService


class QuestRoomsService:
    def __init__(self, uow_factory: SqlAlchemyUnitOfWork) -> None:
        self.__uow = uow_factory
        self.__CertService = CertificateService(uow_factory)
    ...
    async def check_available_rooms(
        ...
        async with self.__uow as uow:
            ...
            for room in all_rooms:
                if all(room.id != booking.quest_room_id for booking in all_bookings):
    ...
    async def book_room(self, customer_request: CustomerRequest):
        ...
        async with self.__uow as uow:
            ...
            available_rooms = await self.check_available_rooms(
                rooms_repo, bookings_repo
            )
